    ...
    if (turtle_spawning_service_ready_) {
      if (turtle_spawned_) {
        ...
        try {
          ...
          auto befor = this->now();
          t = tf_buffer_->lookupTransform(toFrameRel, now, fromFrameRel, when,
                                          "world", 50ms);
          auto after = this->now();
          RCLCPP_INFO(this->get_logger(), "%ld",
                      (after.nanoseconds() - befor.nanoseconds()) / 1000000);
        ...
        msg.angular.z = scaleRotationRate * atan2(t.transform.translation.y,
                                                  t.transform.translation.x);

        static const double scaleForwardSpeed = 0.5;
        msg.linear.x =
            scaleForwardSpeed * sqrt(pow(t.transform.translation.x, 2) +
